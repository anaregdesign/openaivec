...
class IntentAnalysis(BaseModel):
    primary_intent: str = Field(description="Primary customer intent: get_help, make_purchase, cancel_service, get_refund, report_issue, seek_information, request_feature, provide_feedback")
    secondary_intents: List[str] = Field(description="Additional intents if multiple goals are present")
    action_required: str = Field(description="Required action: provide_information, troubleshoot, process_request, escalate, redirect, schedule_callback")
    intent_confidence: float = Field(description="Confidence in intent detection (0.0-1.0)")
    success_likelihood: str = Field(description="Likelihood of successful resolution: very_high, high, medium, low, very_low")
